...
async def health(config: Settings = Depends(get_settings)) -> dict[str, str]:
    return {"status": "ok", "mode": config.app_mode, "version": "0.5.0"}
...
@app.post("/api/v5/research-strategy", response_model=ResearchStrategyResponse)
async def create_research_strategy(
    ...
    if config.app_mode == "mock":
        return await service.generate_research_strategy(brief)

    key = f"research-strategy:{brief_cache_key(brief)}"
    cached = usage_guard.cached(key)
    if isinstance(cached, ResearchStrategyResponse):
        response.headers["X-AI-Cache"] = "HIT"
        return cached
    ...
    if not admission.allowed:
        ...
        return await build_service(
            config.model_copy(update={"app_mode": "mock"})
        ).generate_research_strategy(brief)

    try:
        result = await service.generate_research_strategy(brief)
        usage_guard.remember(key, result)
        response.headers["X-AI-Cache"] = "MISS"
        return result
    except InsightServiceError as exc:
