...
flowchart LR
    ...
    M --> O
    O --> Q["Deterministic quality gate"]
    Q --> B --> F
```
...
- Runs deterministic post-generation checks after schema validation
...
sequenceDiagram
    ...
    API->>API: Check research patterns and claim language
...
`quality_review` is computed by the server after the Dify run. It is not generated by the model. Hard structural and evidence inconsistencies fail validation; reviewable wording and research-question problems are returned as field-level warnings.
...
- No user data persistence in V0.2
